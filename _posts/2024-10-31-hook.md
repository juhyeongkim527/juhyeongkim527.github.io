...
**이 부분은 일단 `free(ptr);`이 원래의 수행을 하지 않으면서 종료되지 않으면, 결국 `system("/bin/sh");`을 최종적으로 수행하기 때문인 것 같은데, 1. 왜 인자를 전달해주지 않아도 오류가 발생하지 않는지, 2. 세 원가젯 중 하나만 가능한지는 질문을 올려본 후 다시 공부해보고 작성하겠다.**
